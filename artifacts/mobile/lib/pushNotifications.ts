...
// ─── Expo Go detection ───────────────────────────────────────────────────────
// In Expo SDK 53+ Android push notifications were removed from Expo Go.
// Skip native notification calls when running inside Expo Go to prevent crashes.
const isExpoGo: boolean =
  (Constants as any).executionEnvironment === "storeClient" ||
  (Constants as any).appOwnership === "expo";

// ─── Configure foreground notification behaviour ──────────────────────────────
// Only configure when NOT in Expo Go (native module unavailable in Expo Go)
if (!isExpoGo) {
...
export async function registerForPushNotifications(userId: string): Promise<string | null> {
  ...
  if (isExpoGo) return null; // skip push registration in Expo Go
